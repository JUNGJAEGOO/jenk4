...
> ※ 본 문서는 alpha 개발 단계의 문서입니다. 사용에 관심이 있으신 분은 **support@cloud.toast.com**으로 문의해 주시기 바랍니다.
...
![image](http://static.toastoven.net/prod_apigateway/img_13.png)

<center>[그림2] Domain 리스트</center>

웹콘솔의 [API Gateway] > [API Setting] 을 클릭하여 Domain 관리 화면으로 접근할 수 있습니다.

![image](http://static.toastoven.net/prod_apigateway/img_14.png)
...
![image](http://static.toastoven.net/prod_apigateway/img_15.png)

<center>[그림4] Endpoint 리스트</center>

Domain 목록에서 관리할 Domain의 가장 우측에 있는 [Setting] > [Endpoint]를 클릭하여 Endpoint 관리 화면으로 접근할 수 있습니다.

![image](http://static.toastoven.net/prod_apigateway/img_16.png)
...
![image](http://static.toastoven.net/prod_apigateway/img_17.png)

<center>[그림6] API 통계 페이지</center>

![image](http://static.toastoven.net/prod_apigateway/img_18.png)

<center>[그림7] API 통계 > UrlSuffix 별 통계</center>

웹콘솔에서 [API Gateway] > [Dashboard]를 클릭하여 통계 화면으로 접근할 수 있습니다.

![image](http://static.toastoven.net/prod_apigateway/img_19.png)
...
![image](http://static.toastoven.net/prod_apigateway/img_20.png)

<center>[그림9] Domain 생성</center>

2.Domain 설정 폼을 채우고 [Save] 버튼을 클릭하여 Domain을 생성합니다.

![image](http://static.toastoven.net/prod_apigateway/img_21.png)
...
![image](http://static.toastoven.net/prod_apigateway/img_22.png)

<center>[그림11] Domain 편집</center>

2.Domain 설정을 변경하고 [Save] 버튼을 클릭하여 저장합니다.

![image](http://static.toastoven.net/prod_apigateway/img_23.png)
...
![image](http://static.toastoven.net/prod_apigateway/img_24.png)
...
![image](http://static.toastoven.net/prod_apigateway/img_26.png)

<center>[그림15] Endpoint 생성</center>

2.빈 칸을 채우고 우측에 있는 [Save] 버튼을 클릭하여 Endpoint를 생성합니다.

![image](http://static.toastoven.net/prod_apigateway/img_27.png)
...
![image](http://static.toastoven.net/prod_apigateway/img_28.png)

<center>[그림17] Endpoint 편집</center>

2.설정을 수정하고 우측에 있는 [Save] 버튼을 클릭하여 저장합니다.

![image](http://static.toastoven.net/prod_apigateway/img_29.png)
...
![image](http://static.toastoven.net/prod_apigateway/img_30.png)
...
![image](http://static.toastoven.net/prod_apigateway/img_31.png)

<center>[그림20] Endpoint Plugin 추가</center>

2.Plugin 설정 폼을 채우고 우측에 있는 [Save] 버튼을 클릭하여 저장합니다.

![image](http://static.toastoven.net/prod_apigateway/img_32.png)
...
![image](http://static.toastoven.net/prod_apigateway/img_33.png)

<center>[그림22] Endpoint Plugin 편집</center>

2.설정을 수정하고 우측에 보이는 [Save] 버튼을 클릭하여 저장합니다.

![image](http://static.toastoven.net/prod_apigateway/img_34.png)
...
![image](http://static.toastoven.net/prod_apigateway/img_35.png)

<center>[그림24] Endpoint Plugin 편집</center>

2.우측에 보이는 [Delete] 버튼을 클릭하여 삭제합니다.

![image](http://static.toastoven.net/prod_apigateway/img_36.png)
...
![image](http://static.toastoven.net/prod_apigateway/img_12.png)
...
IP 기반 Access Control 기능 입니다. 특정 IP를 allow하거나 deny할 수 있습니다.
...
1.  [API Gateway > API Setting] 에서 도메인 셋팅을 위한 화면으로 이동합니다. ![image](http://static.toastoven.net/prod_apigateway/img_plugin_maintenance_1.png)

    <center>[그림27] 도메인 셋팅 이동</center>

2.  [Plugin Setting > Maintenance] 에서 Maintenance Response 플러그인을 추가합니다. ![image](http://static.toastoven.net/prod_apigateway/img_plugin_maintenance_2.png)
...
1.  [API Gateway > API Setting] 에서 도메인 셋팅을 위한 화면으로 이동합니다. ![image](http://static.toastoven.net/prod_apigateway/img_plugin_hmac_1.png)

    <center>[그림29] 도메인 셋팅 이동</center>

2.  [Plugin Setting > Authentication] 에서 HMAC 플러그인을 추가합니다. ![image](http://static.toastoven.net/prod_apigateway/img_plugin_hmac_2.png)

    <center>[그림30] HMAC 플러그인 설정</center>

> [참고] Clock skew 설정 APIGW 서버의 시간과 Client에서 보낸 X-TC-Timestamp 사이의 차가 Clock Skew보다 크면 HMAC 인증에 실패하게 됩니다. Clock Skew 값은 0~86400(sec)이며, 만약 0이라면 Clock Skew를 무시합니다.
...
| 요청 | StringToSign |
| --- | --- |
| GET /test/1?query1=1&query2=2

X-TC-Timestamp: 2016-07-23T12:20:02+09:00
<span style="color:red">Authorization: IqY8u/RZY8IMESwa/TPW9P9Z39Y=</span> | GET\n
/test/1?query1=1&query2=2\n
2016-07-23T12:20:02+09:00 |
...
1.  [API Gateway > API Setting] 에서 도메인 셋팅을 위한 화면으로 이동합니다. ![image](http://static.toastoven.net/prod_apigateway/img_plugin_jwt_1.png)

    <center>[그림31] 도메인 셋팅 이동</center>

2.  [Plugin Setting > Authentication] 에서 JWT 플러그인을 추가합니다. ![image](http://static.toastoven.net/prod_apigateway/img_plugin_jwt_2.png)

    <center>[그림32] JWT 플러그인 설정</center>

> [참고] APIGW 서버의 시간과 Client에서 보낸 ExpirationTime 사이의 차가 Clock Skew보다 크면 JWT인증에 실패하게 됩니다. Clock Skew 값은 0~86400 사이의 값을 입력할 수 있습니다.
...
Request: GET /test/1?query1=1&query2=2
...
1.  [API Gateway > API Setting] 에서 도메인 셋팅을 위한 화면으로 이동합니다. ![image](http://static.toastoven.net/prod_apigateway/img_plugin_cors_1.png)

    <center>[그림33] 도메인 셋팅 이동</center>

2.  [Plugin Setting > CORS] 에서 CORS 플러그인을 추가합니다. ![image](http://static.toastoven.net/prod_apigateway/img_plugin_cors_2.png)

    <center>[그림34] CORS 플러그인 설정</center>

3.  Allowed credentials: Request with Credential 방식을 사용할 수 있는지를 지정합니다.

4.  Max credentials Age: Preflight Request의 결과가 캐시에 얼마나 오래 남아 있는지를 지정합니다. 초 단위이며 0~86400 사이의 값을 입력할 수 있습니다.

5.  Allowed origins: 지정된 도메인만 서버의 리소스에 접근할 수 있도록 지정할 수 있습니다. *로 입력할 경우 모든 도메인에 대해서 허용합니다. (단, *로 지정할 경우 credentials를 지원하지 않으므로 구체적인 도메인을 지정하셔야 합니다.) 지정된 도메인에서만 허용하도록 할 경우 ,(comma)로 분리하여 입력합니다. 도메인은 URI(scheme, domain, port) 포맷으로 입력해야 합니다.(ex: http://api-gw.toast.com:8080)

6.  Allowed methods: 지정된 HTTP Method만 서버 리소스의 접근을 허용합니다.

7.  Allowed headers: 서버의 리소스의 접근을 허용할 HTTP Method를 지정합니다. 여러 Method를 입력할 경우 ,(comma)로 분리하여 입력합니다.

8.  Exposed headers: 브라우저에서 접근할 수 있는 허용 헤더를 지정합니다. 여러 헤더를 입력할 경우 ,(comma)로 분리하여 입력합니다.

9.  자세한 CORS 규약은 https://www.w3.org/TR/cors/ 를 참고 해주세요.
...
Pre API의 응답코드에 따라 200이면 Endpoint를 호출하고, 응답코드가 200이 아니면 Pre API의 응답결과를 리턴합니다. 만약, Pre API 호출에 실패할 경우 에러를 리턴하게 됩니다.

#### Pre API 설정

1.  [API Gateway > Endpoint] 화면으로 이동 ![image](http://static.toastoven.net/prod_apigateway/img_plugin_preapi_1.png)

    <center>[그림35] Endpoint 설정 화면 이동</center>

2.  Plugins > Pre API를 추가합니다. ![image](http://static.toastoven.net/prod_apigateway/img_plugin_preapi_2.png)

    <center>[그림36] Pre API 플러그인 추가</center>

3.  호출한 Method type과 URL을 입력한 후 저장합니다. ![image](http://static.toastoven.net/prod_apigateway/img_plugin_preapi_3.png)
...
> [참고] 설정한 헤더 키가 이미 존재한다면 덮어쓰게 됩니다.

#### Modify Headers 설정

1.  [API Gateway > Endpoint] 화면으로 이동합니다. ![image](http://static.toastoven.net/prod_apigateway/img_plugin_modifyheaders_1.png)

    <center>[그림38] Endpoint 설정 화면 이동</center>

2.  Plugins > Modify Headers 플러그인을 추가합니다. ![image](http://static.toastoven.net/prod_apigateway/img_plugin_modifyheaders_2.png)

    <center>[그림39] Modify Headers 플러그인 추가</center>

3.  Plugins > Modify Headers 플러그인 설정 정보를 입력합니다. ![image](http://static.toastoven.net/prod_apigateway/img_plugin_modifyheaders_3.png)

    <center>[그림40] Modify Headers 플러그인 설정 정보 입력</center>

4.  Request Headers는 요청 헤더를 수정할 수 있습니다.

5.  Response Headers는 응답 헤더를 수정할 수 있습니다.
...
1.  [API Gateway > Endpoint] 화면으로 이동합니다. ![image](http://static.toastoven.net/prod_apigateway/img_plugin_usagequota_1.png)

    <center>[그림41] Endpoint 설정 화면 이동</center>

2.  Plugins > Usage Quota 플러그인을 추가한 후 단위 시간(sec) 동안 최대 호출 가능 횟수를 입력합니다. ![image](http://static.toastoven.net/prod_apigateway/img_plugin_usagequota_2.png)
